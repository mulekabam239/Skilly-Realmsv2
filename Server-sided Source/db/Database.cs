...
namespace db
{
    public partial class Database : IDisposable
    {
        ...
        {
            "Zenakis", "Kelix", "Zelontas", "Parink", "Siger", "Lonhi"
        };
...
AND characters.charId=death.chrId;";
        ...
        {
            var cmd = CreateQuery();
            cmd.CommandText = "SELECT * FROM accounts WHERE uuid=@uuid AND password=@password;";
        ...
        public Account Register(string uuid, string password, bool isGuest, XmlData data)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO accounts(uuid, password, name, rank, namechosen, verified, guild, guildRank, guildFame, vaultCount, maxCharSlot, regTime, guest, banned, locked, ignored, isAgeVerified, authToken) VALUES(@uuid, @password, @randomName, @rank, 0, 0, 0, 0, 0, 1, 2, @regTime, @guest, 0, @empty, @empty, 1, @authToken);";
            cmd.Parameters.AddWithValue("@uuid", uuid);
