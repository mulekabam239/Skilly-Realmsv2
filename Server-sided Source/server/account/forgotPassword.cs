            ...
            using (Database db = new Database())
            {
                ...
                if (cmd.ExecuteNonQuery() == 1)
                {
                    ...
                    message.Body = emailBody.
                        Replace("{RPLINK}", String.Format("{0}/{1}{2}", Program.Settings.GetValue<string>("serverDomain", "127.0.0.1"), "account/resetPassword?authToken=", authKey)).
                        Replace("{SUPPORTLINK}", String.Format("{0}", Program.Settings.GetValue<string>("supportLink", "127.0.0.1"))).
                        Replace("{SERVERDOMAIN}", Program.Settings.GetValue<string>("serverDomain", "127.0.0.1"));
