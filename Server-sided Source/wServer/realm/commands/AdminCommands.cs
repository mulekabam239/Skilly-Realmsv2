...
namespace wServer.realm.commands
{
    ...
    internal class SetFameCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
                player.SendHelp("Usage: /setfame <famw>");
                return false;
            }
            player.Manager.Database.DoActionAsync(db =>
            {
                var cmd = db.CreateQuery();
                cmd.CommandText = "UPDATE `stats` SET `fame`=@fam WHERE accId=@accId";
                cmd.Parameters.AddWithValue("@fam", args[0]);
                cmd.Parameters.AddWithValue("@accId", player.AccountId);
                player.UpdateCount++;
                if (cmd.ExecuteNonQuery() == 0)
                {
    ...
    internal class SpawnCommand : Command
    {
        ...
        {
            int num;
            if (!(player.Owner.Name == "SpawningArena"))


                {
                player.SendError("Go into the spawning arena in Nexus!");
                return false;
            }
            if (args.Length > 0 && int.TryParse(args[0], out num)) //multi
            {
                ...
                if (!(player.Client.Account.Rank > 3) && c > 1)
                {
                    player.SendError("Maximum spawn count is set to 1!");
                    return false;
                }
                if (player.Client.Account.Rank > 7 && c > 1)
                {
                ...
                foreach (Client i in player.Manager.Clients.Values)
                {
                    i.SendPacket(new TextPacket
                    {
                        ...
                        Stars = 37,
                        Name = "#Who Spawned",
                        Text = "Someone just spawned something..."
                    });
                }
            }
    ...
    internal class Gold : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (args[0] == "")
            {
                player.SendHelp("Usage: /gold <Gold Amount>");
                return false;
            }
            try
            {
                int newGold = Convert.ToInt32(args[0]);
                player.Credits = newGold;
                player.SaveToCharacter();
               


                player.UpdateCount++;
                player.SendInfo("Updated Gold To: " + newGold);
            }
            catch
            {
                player.SendInfo("Error Setting Gold");
                return false;
            }
            return true;
        }
    ...
    internal class GimmeCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (!icdatas.TryGetValue(name, out objType))
            {
                player.SendError("Item doesn't exist!");
                return false;
            }
    ...
    internal class GiveCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (!icdatas.TryGetValue(name, out objType))
            {
                player.SendError("Item does not exist!");
                return false;
            }
            if (!player.Manager.GameData.Items[objType].Secret || player.Client.Account.Rank >= 2)
            {
    ...
    internal class GlandCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (!(player.Owner.Name == "Realm"))


            {
                player.SendError("You can only use this command in a Realm!");
                return false;
            }
            if (args.Length == 0)
            {
                player.SendHelp("Usage: /glands");
            }
            else
            {
                int x, y;
    ...
    {
        public KillAll() : base("killAll", permLevel: 6) { }
    ...
    internal class NewsCommand : Command
    {
        public NewsCommand() : base("news") { }

        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            player.SendInfo("Hey, 'walkin faggot' redirected you here!");
            player.SendInfo("This is news section");
            player.SendInfo("added /news command xDD");
            return true;
        }
    }
                ...
                foreach (KeyValuePair<int, World> w in player.Manager.Worlds)
                {
                    ...
                    if (w.Key != 0)
                    {
                        world.BroadcastPacket(new TextPacket
                        {
                            ...
                            BubbleTime = 0,
                            Text =
                                "Server will be restarted in around 10 seconds, come to nexus to save your progress."
                        }, null);
                    }
                }
            }
    ...
    {
        public ListCommands() : base("commands", permLevel: 4) { }
