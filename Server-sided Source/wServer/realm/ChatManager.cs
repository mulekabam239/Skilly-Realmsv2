...
namespace wServer.realm
{
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            ...
            {
                Name = (src.Client.Account.Rank >= 7 ? "#[Admin] " : src.Client.Account.Rank >= 6 ? "#[Co-Admin] " : src.Client.Account.Rank >= 5 ? "[VIP] " : src.Client.Account.Rank >= 4 ? "[Legendary] " : src.Client.Account.Rank >= 3 ? "[Super Donator] " : src.Client.Account.Rank >= 2 ? "@[VIP] " : src.Client.Account.Rank >= 1 ? "[Donator] " : src.Client.Account.Rank >= 0 ? "[Player] " : "") + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
                Recipient = "",
