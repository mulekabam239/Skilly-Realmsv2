        ...
        public bool Activate(RealmTime time, Item item, UseItemPacket pkt)
        {
            ...
            foreach (ActivateEffect eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.MysteryChest:
                        string[] items = new[]
                        {
                            "Energy Staff",
                            "Shortbow"
                        ...
                        Owner.BroadcastPacket(new TextPacket
                        {
                            ...
                            Stars = 70,
                            Name = "#Chest Manager",
                            Text = Name + " just opened a mystery chest!"
                        }, null);
                    ...
                    case ActivateEffects.MysteryPortal:
                        ...
                        var descss = Manager.GameData.Portals.Where(_ => dungeons.Contains<string>(_.Value.ObjectId)).Select(_ => _.Value).ToArray();
                        var portalDesc = descss[Random.Next(0, descss.Count())];
                        Entity porr = Entity.Resolve(Manager, portalDesc.ObjectId);
                        porr.Move(this.X, this.Y);
                        Owner.EnterWorld(porr);
